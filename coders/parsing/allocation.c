...
/*   Updated: 2026/04/05 11:07:51 by bclairot         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_coders(t_data *data, int nbr_of_coders)
{
	...
	if (!data->coders)
		return (error(1,
				"Problem with the malloc in init_coders, "
...
int	init_dongles(t_data *data, int nbr_of_coders)
{
	...
	if (!data->dongle)
	{
		free(data->coders);
		return (error(1,
				"Problem with the malloc in init_dongles, "
				"close some applications and retry."));
	}
	while (i < nbr_of_coders)
	{
...
int	init_threads(t_data *data, pthread_t **threads)
{
	...
	if (!(*threads))
	{
		...
		free(data->dongle);
		return (error(1,
				"Problem with the malloc in init_threads,"
				" close some applications and retry."));
	}
		...
		t_thread_data **threads_data)
{
	...
	if (!*threads_data)
	{
		...
		free(data->dongle);
		return (error(1,
				"Problem with the malloc in init_threads_data,"
				" close some applications and retry."));
	}
